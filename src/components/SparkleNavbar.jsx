...
const SparkleNavbar = ({
    items,
    color = "#F7931E", // Brand Orange default
}) => {
    ...
    // Track scroll position and update active index based on visible section
    useEffect(() => {
        ...
        const observerCallback = (entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    // Find which nav item corresponds to this section
                    const sectionId = entry.target.getAttribute('id');
                    const index = items.findIndex(item => item.href === `#${sectionId}`);

                    if (index !== -1 && index !== activeIndex) {
                        // Only update if changed and there's a button to animate to
                        const newButton = buttonRefs.current[index];
                        if (newButton && activeElementRef.current) {
                            setActiveIndex(index);

                            // Smoothly move the active indicator without the full animation
                            gsap.to(activeElementRef.current, {
        ...
        // Observe all sections referenced by nav items
        items.forEach(item => {
    ...
    const handleClick = (index, item) => {
        ...
        if (
            ...
            // If clicking current tab, still handle navigation scroll
            if (item.href) {
    ...
    return (
        ...
        .navigation-menu ul li:not(.active):hover button {
          color: ${color};
          text-shadow: 0 0 10px ${color}40, 0 0 20px ${color}20;
        }
        ...
        .navigation-menu .active-element .strike {
          ...
          mix-blend-mode: screen; /* Changed from multiply */
        }
