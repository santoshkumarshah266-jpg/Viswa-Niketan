...
const FancyButton = ({
    ...
    // Extract text from children if primaryText is not provided
    let displayText = primaryText;
    if (!displayText && typeof children === 'string') {
        displayText = children;
    } else if (!displayText && React.Children.count(children) > 0) {
    ...
    // Default secondary text is the same as primary
    const secondText = secondaryText || displayText;

    // Split text into individual characters with index
    const renderCharacters = (text, stateClass) => {
        if (!text) return null;
    ...
    return (
        <button className={`fancy-button ${className}`} onClick={onClick} type={type} {...props}>
            ...
            <div className="fancy-wrap">
                ...
                <div className="fancy-content">
                    {renderCharacters(displayText, 'state-1')}
                    ...
                    {renderCharacters(secondText, 'state-2')}
                </div>
            </div>
        </button>
    );
